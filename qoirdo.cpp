...
struct smooth_desc {
	...
	float smooth_max_mse_scale {/*DEF_SMOOTH_MAX_MSE_SCALE*/ 2500}; // help claims 250png 2500qoi
	float max_ultra_smooth_std_dev {DEF_MAX_ULTRA_SMOOTH_STD_DEV};
	float ultra_smooth_max_mse_scale {/*DEF_ULTRA_SMOOTH_MAX_MSE_SCALE*/ 2500}; // 1500 (PNG) or 2500 (QOI)
};
...
std::vector<uint8_t> encode_qoi_rdo_simple(const uint8_t* data, const qoi_rdo_desc& desc, int quality) {
	...
	//const uint32_t rdo_qoi_len = output_data.size();
